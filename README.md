...
- `spot-history` - plots the **`<currency>_USDC` spot pair** as a daily candlestick chart (trailing year, ~180-day default window) - the market-context strip above the tabs.
...
| GET    | `/api/spot/history?currency=BTC`            | Daily spot OHLCV (trailing year)     |
